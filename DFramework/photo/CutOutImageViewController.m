...
#import "CutOutImageViewController.h"
...
@interface CutOutImageViewController ()
...
@implementation CutOutImageViewController
...
#ifdef IN_QuickManHR
    KRButton *cancelBtn = [[KRButton alloc] initWithFrame: CGRectMake(13, self.view.frame.size.height - 50.0f, 125, 40)
                                                                       type:kKRButtonRed
                                                                      title: @"取消"];
#else
...
#ifdef IN_QuickManHR
    KRButton *confirmBtn = [[KRButton alloc] initWithFrame: CGRectMake(self.view.frame.size.width - cancelBtn.right, cancelBtn.top, cancelBtn.width, cancelBtn.height)
                                                                        type: kKRButtonGreen
                                                                       title: @"确认"];
#else
...
- (void)cancel:(id)sender {
    if (self.delegate && [self.delegate conformsToProtocol:@protocol(CutOutImageDelegate)]) {
        [self.delegate imageCutOutDidCancel:self];
    }
}

- (void)confirm:(id)sender {
    if (self.delegate && [self.delegate conformsToProtocol:@protocol(CutOutImageDelegate)]) {
        [self.delegate imageCutOut:self didFinished:[self getSubImage]];
    }
}
